...
void EncodeJob::run()
{
    ...
    if (targetFormat._format == bridge::RtpMap::Format::OPUS)
    {
        ...
        if (_outboundContext.rtpMap._absSendTimeExtId.isSet())
        {
            rtp::GeneralExtension1Byteheader absSendTime(_outboundContext.rtpMap._absSendTimeExtId.get(), 3);
            extensionHead.addExtension(cursor, absSendTime);
        }
        if (_outboundContext.rtpMap._audioLevelExtId.isSet())
        {
            rtp::GeneralExtension1Byteheader audioLevel(_outboundContext.rtpMap._audioLevelExtId.get(), 1);
