...
{
    const size_t inSamplesPerPacket = 8000 / 50;
    const size_t outSamplesPerPacket = 48000 / 50;
    auto resampler = codec::createPcmResampler(outSamplesPerPacket, 8000, 48000);

    int16_t upsampled[outSamplesPerPacket];
    for (int i = 0; i < 3; ++i)
    {
        size_t produced = resampler->resample(_pcmData + inSamplesPerPacket * i, inSamplesPerPacket, upsampled);
...
{
    const size_t inSamplesPerPacket = 48000 / 50;
    const size_t outSamplesPerPacket = 8000 / 50;
    auto resampler = codec::createPcmResampler(inSamplesPerPacket, 48000, 8000);

    int16_t downsampled[outSamplesPerPacket];
    for (int i = 0; i < 3; ++i)
    {
        size_t produced = resampler->resample(_pcm48kHzData + inSamplesPerPacket * i, inSamplesPerPacket, downsampled);
        EXPECT_EQ(produced, outSamplesPerPacket);
    }
}
