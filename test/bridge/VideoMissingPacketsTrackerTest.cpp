...
class VideoMissingPacketsTrackerTest : public ::testing::Test
{
    void SetUp() override
    {
        _videoMissingPacketsTracker = std::make_unique<bridge::VideoMissingPacketsTracker>(0);
    }
...
{
    _videoMissingPacketsTracker->onMissingPacket(1, 1);
    _videoMissingPacketsTracker->onMissingPacket(2, 1);
    ...
    numMissingSequenceNumbers = _videoMissingPacketsTracker->process(1000, 100, missingSequenceNumbers);
    ...
    numMissingSequenceNumbers = _videoMissingPacketsTracker->process(2000, 100, missingSequenceNumbers);
    EXPECT_EQ(2, numMissingSequenceNumbers);
    EXPECT_EQ(1, missingSequenceNumbers[0]);
    EXPECT_EQ(2, missingSequenceNumbers[1]);
    ...
    numMissingSequenceNumbers = _videoMissingPacketsTracker->process(3000, 100, missingSequenceNumbers);
    EXPECT_EQ(1, numMissingSequenceNumbers);
    EXPECT_EQ(1, missingSequenceNumbers[0]);

    for (auto i = 0; i < 10; ++i)
    {
        _videoMissingPacketsTracker->process(4000 + i * 1000, 100, missingSequenceNumbers);
    }

    numMissingSequenceNumbers = _videoMissingPacketsTracker->process(1100, 100, missingSequenceNumbers);
    EXPECT_EQ(0, numMissingSequenceNumbers);
}
