...
{
    _unackedPacketsTracker->onPacketSent(1, 1);
    _unackedPacketsTracker->onPacketSent(2, 1);

    std::array<uint16_t, bridge::UnackedPacketsTracker::maxUnackedPackets> unackedSequenceNumbers{};
    auto numUnackedSequenceNumbers = _unackedPacketsTracker->process(1, unackedSequenceNumbers);
    EXPECT_EQ(0, numUnackedSequenceNumbers);

    numUnackedSequenceNumbers = _unackedPacketsTracker->process(1000, unackedSequenceNumbers);
    ...
    numUnackedSequenceNumbers = _unackedPacketsTracker->process(2000, unackedSequenceNumbers);
    EXPECT_EQ(2, numUnackedSequenceNumbers);
    EXPECT_EQ(1, unackedSequenceNumbers[0]);
    EXPECT_EQ(2, unackedSequenceNumbers[1]);
    ...
    numUnackedSequenceNumbers = _unackedPacketsTracker->process(3000, unackedSequenceNumbers);
    EXPECT_EQ(1, numUnackedSequenceNumbers);
    EXPECT_EQ(1, unackedSequenceNumbers[0]);

    for (auto i = 0; i < 10; ++i)
    {
        _unackedPacketsTracker->process(4000 + i * 1000, unackedSequenceNumbers);
    }

    numUnackedSequenceNumbers = _unackedPacketsTracker->process(1100, unackedSequenceNumbers);
    EXPECT_EQ(0, numUnackedSequenceNumbers);
}
