    ...
    const uint64_t timestamp)
{
    ...
    const auto rtpFrequency = _audio.constainsPayload(rtpHeader->payloadType) ? _audio.rtpFrequency : 90000;
...
void TransportImpl::protectAndSend(memory::UniquePacket packet)
{
    ...
    if (rtp::isRtpPacket(*packet))
    {
        ...
        const auto isAudio = (payloadType <= 8 || _audio.constainsPayload(payloadType));
