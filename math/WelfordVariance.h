...
template <typename T>
class RollingVarianceTrack
{
public:
    RollingVarianceTrack(uint32_t windowSize)
        : _windowSize(windowSize),
          _mean(0),
          _count(0),
          _cursor(0),
    ...
    ~RollingVarianceTrack() { delete[] _values; }

    void add(T value, T mean)
    {
        _cursor = (_cursor + 1) % _windowSize;

        if (_count >= _windowSize)
        {
            const auto newVarValue = (value - _mean) * (value - mean);
            _varianceAcc += newVarValue - _values[_cursor];
            _mean = mean;
            _values[_cursor] = newVarValue;
        }
        else
        {
            ++_count;
